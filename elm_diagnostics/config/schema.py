...
class WaterBalanceConfig(BaseModel):
    storages: list[str] = Field(default_factory=lambda: [
    ...
    outputs: list[str] = Field(default_factory=lambda: [
        "QFLX_EVAP_TOT", "QOVER", "QDRAI", "QDRAI_PERCH",
        "QFLX_SUB_SNOW", "QSNOMELT"
    ])
    et_components: list[str] = Field(default_factory=lambda: [
        "QFLX_EVAP_VEG", "QFLX_EVAP_GRND", "QFLX_SUB_SNOW"
    ])
...
class CH4Config(BaseModel):
    aerenchyma: list[str] = Field(default_factory=lambda: [
        "CH4_SURF_AERE_SAT", "CH4_SURF_AERE_UNSAT"
    ])
    diffusion: list[str] = Field(default_factory=lambda: [
        "CH4_SURF_DIFF_SAT", "CH4_SURF_DIFF_UNSAT"
    ])
    ebullition: list[str] = Field(default_factory=lambda: [
...
class CarbonBalanceConfig(BaseModel):
    ...
    pools: list[str] = Field(default_factory=lambda: [
        "LEAFC", "LIVESTEMC", "DEADSTEMC", "FROOTC", "LIVECROOTC",
        "DEADCROOTC", "TOTSOMC", "TOTLITC", "CWDC"
    ])
    fluxes: list[str] = Field(default_factory=lambda: [
        "GPP", "AR", "HR", "ER", "NEE", "TOTFIRE", "WOOD_HARVESTC"
    ])
...
class EnergyBalanceConfig(BaseModel):
    radiation: list[str] = Field(default_factory=lambda: [
        "FSDS", "FSR", "FLDS", "FIRE", "FSA", "FIRA"
    ])
    turbulent: list[str] = Field(default_factory=lambda: ["FSH", "EFLX_LH_TOT"])
    ground: list[str] = Field(default_factory=lambda: ["FGR"])
    storage: list[str] = Field(default_factory=lambda: ["hc_soisno"])
    frame: Literal["water_year", "calendar"] = "calendar"
